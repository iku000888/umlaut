(ns umlaut.generators.lacinia
 ...
 (defn- filter-input-nodes [nodes]
   (filter (annotation-comparer space "identifier" "")
           nodes))
 
 (defn- filter-nodes-!mutation!query [nodes]
   (filter (fn [node]
             (not
              (or
               ((annotation-comparer space "identifier" "mutation") node)
               ((annotation-comparer space "identifier" "query") node))))
           nodes))
 ...
 (defn suffix-input-types-with-Input [input-objects-map]
   (let [smap (->> input-objects-map
                   keys
                   (map (fn [k]
                          [k (keyword (str (name k) "Input"))]))
                   (into {}))]
     (cw/postwalk-replace
      smap
      input-objects-map)))
     ...
     (update
      (as-> nodes-seq coll
        ...
        (reduce
         ...
         coll (filter-nodes-!mutation!query nodes-seq))
        (reduce
         (fn [acc [key node]]
